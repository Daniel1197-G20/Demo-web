...
export default function Dashboard() {
  ...
  // Cached stats query with stale-while-revalidate
  const { data: statsData, isLoading } = useCachedData(
    'admin:stats',
    () => store.getStats(),
    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 sm:p-7 rounded-3xl border border-[#F7DCE5] shadow-[0_4px_20px_rgba(232,44,124,0.04)]">
        ...
        <div className="flex flex-wrap items-center gap-2.5 shrink-0">
          <Tooltip content="Upload photo and publish a new treat" position="bottom">
            <Link to="/admin/products/new">
              <button
                type="button"
                className="flex items-center gap-2 px-5 py-3 rounded-full bg-[#E82C7C] hover:bg-[#D31665] text-white text-xs font-bold shadow-[0_4px_14px_rgba(232,44,124,0.3)] transition-all active:scale-95 focus-ring"
              >
          ...
          <Tooltip content="Review catering requests & formulate quotes" position="bottom">
      ...
      ───────────────────────────────────────────────────────────── */}
      {isLoading ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-6" aria-busy="true">
          <SkeletonStatCard />
          <SkeletonStatCard />
          <SkeletonStatCard />
          <SkeletonStatCard />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
        ...
        <div className="lg:col-span-7 bg-white rounded-3xl border border-[#F7DCE5] p-5 sm:p-7 shadow-[0_4px_20px_rgba(232,44,124,0.04)] space-y-4">
          <div className="flex items-center justify-between border-b border-[#F7DCE5] pb-4">
            ...
            <Tooltip content="Open full product catalog" position="left">
              <Link
                to="/admin/products"
                ...
                {stats.recentProducts.map((p) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col items-end gap-1.5 shrink-0">
                      ...
                      <Tooltip content="Edit treat details & pricing" position="left">
                        <Link
                    ...
                    {stats.recentProducts.map((p) => (
                      <tr key={p.id} className="hover:bg-[#FFF5F8]/40 transition-colors">
                        ...
                        <td className="py-3 text-right">
                          <Tooltip content={`Edit ${p.name}`} position="left">
                            <Link
                              to={`/admin/products/${p.id}/edit`}
                              className="px-3 py-1 rounded-full text-xs font-bold text-[#E82C7C] bg-[#FFF5F8] border border-[#FCE4EC] hover:bg-[#FCE4EC] transition-colors focus-ring"
                            >
                              Edit
                            </Link>
                          </Tooltip>
        ...
        <div className="lg:col-span-5 bg-white rounded-3xl border border-[#F7DCE5] p-5 sm:p-7 shadow-[0_4px_20px_rgba(232,44,124,0.04)] space-y-4">
          <div className="flex items-center justify-between border-b border-[#F7DCE5] pb-4">
            ...
            <Tooltip content="View all wedding & event bookings" position="left">
              <Link
                to="/admin/bookings"
                className="text-xs font-bold text-[#E82C7C] hover:underline flex items-center gap-1 focus-ring rounded-lg px-1"
              >
              ...
              {stats.recentBookings.map((b) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-[11px] pt-1 border-t border-[#F7DCE5]/60">
                    ...
                    <Tooltip content={`Formulate quote proposal for ${b.customer}`} position="top">
                      <Link
                        to={`/admin/bookings/${b.bookingNumber}`}
                        className="text-[#E82C7C] font-bold hover:underline flex items-center gap-1 focus-ring rounded-md px-1"
                      >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Tooltip content="Add a new bakery product to the online shop" position="top">
          <Link
            to="/admin/products/new"
            className="p-5 rounded-3xl bg-white border border-[#F7DCE5] hover:border-[#E82C7C] hover:shadow-[0_8px_25px_rgba(232,44,124,0.08)] transition-all flex items-center gap-4 group focus-ring"
          >
        ...
        <Tooltip content="Organize product collections & navigation filters" position="top">
          <Link
            to="/admin/categories"
            className="p-5 rounded-3xl bg-white border border-[#F7DCE5] hover:border-[#E82C7C] hover:shadow-[0_8px_25px_rgba(232,44,124,0.08)] transition-all flex items-center gap-4 group focus-ring"
          >
        ...
        <Tooltip content="Configure Lagos delivery fees & notification preferences" position="top">
          <Link
            to="/admin/settings"
            className="p-5 rounded-3xl bg-white border border-[#F7DCE5] hover:border-[#E82C7C] hover:shadow-[0_8px_25px_rgba(232,44,124,0.08)] transition-all flex items-center gap-4 group focus-ring"
          >
